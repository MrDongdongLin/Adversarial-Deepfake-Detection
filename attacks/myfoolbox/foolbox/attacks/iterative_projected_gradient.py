...
###################################################################################################################
        def try_epsilon2(epsilon):
            ...
            success = yield from self._run_one2(
                a,
                epsilon,
                optimizer,
                iterations,
...
###################################################################################################################
        ...
        for i in range(k):
            success = yield from try_epsilon2(epsilon)
            if success:
...
########################################################################################################################
    def _run_one2(
        ...
        for _ in range(iterations):
            ...
            # logits, is_adversarial = yield from a.forward_one(x)
            logits, is_adversarial = yield from a.forward_one3(x, confidence=confidence, cmin=a.original_class,
                                                      cmax=a.target_class)
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                if targeted:
                    ce = crossentropy(a.original_class, logits)
...
class MomentumIterativeAttack(
    ...
    def _run_one2(self, *args, **kwargs):
        ...
        success = yield from super(MomentumIterativeAttack, self)._run_one2(
            *args, **kwargs)
        return success
