...
class LBFGSAttack(Attack):
    ...
    def __call__(
        ...
        for i, target_class in enumerate(target_classes):
            self._optimize(a, target_class, epsilon=epsilon, maxiter=maxiter, confidence=confidence)
    ...
    def _optimize(self, a, target_class, epsilon, maxiter, confidence):
        x0 = a.unperturbed
        min_, max_ = a.bounds()
        ...
        if self._approximate_gradient:
            ...
            def crossentropy(x, confidence):
                logits, gradient, is_adversarial = a.forward_and_gradient_one2(
                    x.reshape(shape), target_class, strict=False, return_details=False, confidence=confidence,
                )
            ...
            def loss(x, c, confidence):
                x = x.astype(dtype)
                v1, g1 = distance(x)
                v2, g2 = crossentropy(x, confidence)
                v = v1 + c * v2
                g = g1 + c * g2
...
#######################################################################################################################
        def lbfgsb_2(c, confidence):
            ...
            # _, is_adversarial = a.forward_one3(x.reshape(shape).astype(dtype), confidence=confidence,
            #                                    cmin=a.original_class, cmax=a.target_class)
...
#######################################################################################################################
        ...
        for i in range(30):
            c = 2 * c
            is_adversarial = lbfgsb_2(c, confidence)
            logging.info(
                "Tested c = {:.4e}: {}".format(
        ...
        while c_high - c_low >= epsilon:
            ...
            is_adversarial = lbfgsb_2(c_half, confidence)
            logging.info(
