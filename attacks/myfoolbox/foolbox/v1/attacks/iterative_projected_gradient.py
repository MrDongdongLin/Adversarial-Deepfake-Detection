...
class IterativeProjectedGradientBaseAttack(Attack):
    ...
    def _run(
        ...
        # #########  Wenjie2020/5/15
        # x = a.unperturbed.copy()
        # strict = True
        # gradient = self._gradient(a, x, class_, strict=strict)
        # he = np.sum(np.abs(gradient))
        # with open(r'E:\Wenjie\pycharm\transferability_forensics\random_feature\code_for_server\model\StammNet2_median\attacked\BIM.txt', 'a') as f:
        #     f.write('G_sum={}\n'.format(he))
        # ####################################
...
########################################################################################################################
        ...
        with open('BIM_epsilon.txt', 'a') as f:
            f.write('epsilon={}\n'.format(epsilon))
    ...
    def _run_one(
        ...
        for _ in range(iterations):
            gradient = self._gradient(a, x, class_, strict=strict)
            # Bowen: to avoid computed graident is invalid like inf or non
            if np.inf in gradient or -np.inf in gradient or np.isnan(gradient).any():
                gradient = np.zeros_like(gradient) # no update since now
...
########################################################################################################################
    def _run_one2(
        ...
        for _ in range(iterations):
            gradient = self._gradient(a, x, class_, strict=strict)
            # Bowen: to avoid computed graident is invalid like inf or non (it is not needed for L_infinity)
            # if np.inf in gradient or -np.inf in gradient or np.isnan(gradient).any():
            #     gradient = np.zeros_like(gradient) # no update since now
            # non-strict only for the first call and
            # only if random_start is True
            ...
            # logits, is_adversarial = a.forward_one(x)
            logits, is_adversarial = a.forward_one3(x, confidence=confidence, cmin=a.original_class, cmax=a.target_class)
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                if targeted:
                    ce = crossentropy(a.original_class, logits)
                    logging.debug(
    ...
    def _check_distance(self, a):
        if not isinstance(a.distance, distances.Linfinity):
            logging.warning(
                ...
                " specifying foolboxs.distances.Linfinity as"
    ...
    def _check_distance(self, a):
        if not isinstance(a.distance, distances.MAE):
            logging.warning(
                ...
                " specifying foolboxs.distances.MAE as"
                " the distance metric might lead to suboptimal"
                " results."
            )
    ...
    def _check_distance(self, a):
        if not isinstance(a.distance, distances.MSE):
            logging.warning(
                ...
                " specifying foolboxs.distances.MSE as"
                " the distance metric might lead to suboptimal"
                " results."
            )
...
class LinfinityBasicIterativeAttack(
    ...
    def __call__(
        ...
        self._run(
            a, binary_search, epsilon, stepsize, iterations, random_start, return_early, confidence
        )
...
class MomentumIterativeAttack(
    ...
    def _run_one2(self, *args, **kwargs):
        # reset momentum history every time we restart
        # gradient descent
        self._momentum_history = 0
        return super(MomentumIterativeAttack, self)._run_one2(*args, **kwargs)
