...
########################################################################################################################
    # def forward_one2(self, x, confidence, cmin, cmax, strict=True, return_details=False):
    #     """Interface to model.forward_one for attacks.
    #
    #     Parameters
    #     ----------
    ...
    #     predictions = self.__model.forward_one(x)
    #
    #     logits_diff = predictions[cb] - predictions[ca]
    #     if logits_diff >= confidence:
    #         is_adversarial, is_best, distance = self.__is_adversarial(
    #             x, predictions, in_bounds
    #         )
    #     else:
    ...
    #     return predictions, is_adversarial
########################################################################################################################
    # used for IFGSM to determin how many iterations it will take to find the adv
    def forward_one3_record_steps(self, x, confidence, cmin, cmax, strict=True, return_details=False, s=0):
        """Interface to model.forward_one for attacks.
        ...
        targeted = cmax is not None
        if targeted:
            vec = np.zeros_like(predictions)
            vec[cmax] = np.inf
            ca = np.argmax(predictions - vec)
            cb = cmax
        else:
            vec = np.zeros_like(predictions)
            vec[cmin] = np.inf
            ca = cmin
            cb = np.argmax(predictions - vec)

        logits_diff = np.abs(predictions[cb] - predictions[ca])
        if logits_diff >= confidence:
            is_adversarial, is_best, distance = self.__is_adversarial(
                x, predictions, in_bounds
            )
        ...
        if return_details:
            ...
            return predictions, is_adversarial, s_best

########################################################################################################################

    def forward_one3(self, x, confidence, cmin, cmax, strict=True, return_details=False):
        """Interface to model.forward_one for attacks.
...
########################################################################################################################
    def forward_one3_record_eps(self, x, confidence, cmin, cmax, strict=True, return_details=False):
        in_bounds = self.in_bounds(x)
        assert not strict or in_bounds
        ...
        if return_details:
            ...
            return predictions, is_adversarial, is_best
...
########################################################################################################
    def forward_and_gradient_one2(
        self, x=None, label=None, strict=True, return_details=False, confidence=0,
    ):
        """Interface to model.forward_and_gradient_one for attacks.
        ...
        logits_diff = np.abs(predictions[0] - predictions[1])
        if logits_diff >= confidence:
            is_adversarial, is_best, distance = self.__is_adversarial(
                x, predictions, in_bounds
            )
