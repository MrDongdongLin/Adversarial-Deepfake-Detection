...
from .base import T
from .base import get_criterion, get_is_adversarial
...
class BaseGradientDescent(FixedEpsilonAttack, ABC):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        is_conf = np.zeros_like(classes) > 0
        ...
        for _ in range(self.steps):
            ...
            logits = model(x)
            for i, (success, logit) in enumerate(zip(is_adv, logits)):
                if success:
                    if hasattr(criterion_, "target_classes"):
                        vec = np.zeros_like(logit)
                        vec[classes[i].raw] = np.inf
                        ca = np.argmax(logit - vec)
                        cb = classes[i].raw
                    else:
                        vec = np.zeros_like(logit)
                        vec[classes[i].raw] = np.inf
                        ca = classes[i].raw
                        cb = np.argmax(logit - vec)
                    logits_diff = logit[cb] - logit[ca]
                    if logits_diff >= self.confidence:
                        x_conf.raw[i] = x.raw[i]
                        is_conf[i] = True
            x_out = [x_conf.raw[i] if is_c else x.raw[i] for i, is_c in enumerate(is_conf)]
            x_out = torch.stack(x_out)
            x_out, _ = ep.astensor_(x_out)
            #################################################################

        return restore_type(x), restore_type(is_conf)
