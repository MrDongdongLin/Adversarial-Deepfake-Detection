...
class FixedEpsilonAttack(AttackWithDistance):
    ...
    def __call__(  # type: ignore
        ...
        *,
        epsilons: Union[Sequence[Union[float, None]], float, None],
        **kwargs: Any,
    ) -> Union[Tuple[List[T], List[T], T, List[T]], Tuple[T, T, T, T]]:
        ...
        for epsilon in real_epsilons:
            xp, is_conf = self.run(model, x, criterion, epsilon=epsilon, **kwargs)
        ...
        # success_conf_ = ep.stack(success_conf)
        # assert success_conf_.shape == (K, N)

        xps_ = [restore_type(xp) for xp in xps]
        xpcs_ = [restore_type(xpc) for xpc in xpcs]

        if was_iterable:
            return xps_, xpcs_, restore_type(success_), success_conf
        else:
            assert len(xps_) == 1
            assert len(xpcs_) == 1
            return xps_[0], xpcs_[0], restore_type(success_.squeeze(axis=0)), success_conf
