...
from network.xception_bilstm import Xception_lstm
from network.mesonet import Meso4, MesoInception4
from dataset.transform import xception_default_data_transforms
from dataset.mydataset import MultiDataset
...
def main():
    ...
    writer = SummaryWriter(comment='XL')
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    output_path = os.path.join(root_path, name)
    if not os.path.exists(output_path):
        os.mkdir(output_path)
    torch.backends.cudnn.benchmark = True
    train_dataset = MultiDataset(txt_path=train_list, frame_num=frame_num, transform=xception_default_data_transforms['train'])
    val_dataset = MultiDataset(txt_path=val_list, frame_num=frame_num, transform=xception_default_data_transforms['val'])
    ...
    # model = TruncEfficientLSTM("efficientnet-b3", num_classes=2, pretrained=True)
    model = Xception_lstm(pretrained=True, nc=2)
    if continue_train:
    ...
    for epoch in range(epoches):
        ...
        val_real_size = 0
        val_fake_size = 0
        for i, (image, labels) in tqdm(enumerate(train_loader), total=len(train_loader)):
            iter_loss = 0.0
            iter_corrects = 0.0
            image = image.cuda()
            labels = labels.cuda()
        ...
        model.eval()
        with torch.no_grad():
            for i, (image, labels) in tqdm(enumerate(val_loader), total=len(val_loader)):
                image = image.cuda()
                labels = labels.cuda()
                outputs = model(image)
                _, preds = torch.max(outputs.data, 1)
...
if __name__ == '__main__':
    ...
    parse.add_argument('--root_path', '-r', type=str,
                       # default="/media/hdddati1/donny/Deepfake-Detection-master/outputs",
                       default="D:\\Deepfake\\Methods\\Deepfake-Detection-master\\outputs"
                       )
    parse.add_argument('--name', '-n', type=str, default='xl_c40_299')
    parse.add_argument('--train_list', '-tl', type=str,
                       # default = '/media/hdddati1/donny/Deepfake-Detection-master/data_list/train.txt'
                       default='D:\\Deepfake\\Methods\\Deepfake-Detection-master\\data_list\\c40_train.txt'
                       )
    parse.add_argument('--val_list', '-vl', type=str,
                       # default='/media/hdddati1/donny/Deepfake-Detection-master/data_list/val.txt',
                       default='D:\\Deepfake\\Methods\\Deepfake-Detection-master\\data_list\\c40_val.txt'
                       )
    parse.add_argument('--batch_size', '-bz', type=int, default=32)
    parse.add_argument('--epoches', '-e', type=int, default='20')
    parse.add_argument('--model_name', '-mn', type=str, default='xl_c40_299.pkl')
    parse.add_argument('--continue_train', type=bool, default=False)
    parse.add_argument('--model_path', '-mp', type=str, default='./output/df_xception_c0_299/1_el_c0_299.pkl')
    parse.add_argument('--log_dir', type=str,
                       default='D:\\Deepfake\\Methods\\Deepfake-Detection-master\\log'
                       # default='/media/hdddati1/donny/Deepfake-Detection-master/log'
                       )
